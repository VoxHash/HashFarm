...
- [x] Introduce `monitor/tests/` with pytest for routes, SQLite metrics, and `state.merge_stale_monero`.
- [x] CI runs `ruff`, `pytest`, and `shellcheck` on every pull request to `main`.
